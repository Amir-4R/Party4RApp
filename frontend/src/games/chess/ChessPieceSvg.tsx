...
// =============================================================================
// SVG paths adapted from the public-domain "Cburnett" chess set.
// Supports multiple colour THEMES (classic, royal, ocean) selected at runtime.
// =============================================================================
...
export type ChessTheme = "classic" | "royal" | "ocean";
...
export const CHESS_THEMES: Record<ChessTheme, ChessThemeColors> = {
  // Traditional carved-ivory / carved-ebony look (natural, no eye strain)
  classic: {
    fillWhite: "#FBF1D8",
    fillBlack: "#15161A",
    strokeWhite: "#1A0F05",     // dark brown outline (natural)
    strokeBlack: "#F5DEB1",     // ivory outline (natural)
    shadowWhite: "rgba(0,0,0,0.35)",
    shadowBlack: "rgba(0,0,0,0.55)",
  },
  // Soft gold vs soft lavender (premium, eye-friendly accent)
  royal: {
    fillWhite: "#FFF8E0",
    fillBlack: "#15151C",
    strokeWhite: "#E8C547",     // soft gold
    strokeBlack: "#C4A1F0",     // soft lavender
    shadowWhite: "rgba(232,197,71,0.45)",
    shadowBlack: "rgba(196,161,240,0.45)",
  },
  // Silver vs teal (cool, modern)
  ocean: {
    fillWhite: "#F2F6FA",
    fillBlack: "#0E1A22",
    strokeWhite: "#7FB7D9",     // sky blue
    strokeBlack: "#4FD1C5",     // teal
    shadowWhite: "rgba(127,183,217,0.45)",
    shadowBlack: "rgba(79,209,197,0.45)",
  },
};
...
export default function ChessPieceSvg({ type, color, size, theme = "royal" }: ChessPieceSvgProps) {
  const t = CHESS_THEMES[theme];
  const fill = color === "white" ? t.fillWhite : t.fillBlack;
  const stroke = color === "white" ? t.strokeWhite : t.strokeBlack;
  const shadowColor = color === "white" ? t.shadowWhite : t.shadowBlack;
